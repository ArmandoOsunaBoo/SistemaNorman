...
def upload_employees(ws,wb):
    ...
    for i, row in enumerate(range(ws.nrows)):
            if i < offset:  # (Optionally) skip headers
                continue
            ne = str(ws.cell(i, 1).value)
            ne = ne[:-2]
            try:
                cont=cont+1
                print(cont)
                emp = Employees.objects.get(number=ne)
                if emp:
...
def load_employee(i,ws,wb,employee):
    ...
    employee.clasification_taiwan = ws.cell(i, 56).value
    employee.turn = ws.cell(i, 57).value
    employee.save()
...
def insert_on_excel(wb,ws,employee,row):
    ...
    ws.cell(1, pos).value = "Numero Antiguo"
    c=c+1
    pos=1+pos
    ws.cell(row, c).value = employee.number
    ws.cell(1, pos).value = "Numero"
    c=c+1
    pos=1+pos
